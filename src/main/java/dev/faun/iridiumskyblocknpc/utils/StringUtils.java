...
public class StringUtils {
	 ...
	 *            The text to parse
	 * @param placeholders
	 *            A map of placeholders to replace
	 ...
	 *            The text to parse
	 * @param placeholders
	 *            Array of key-value pairs for placeholders
	 * @return The parsed Component with placeholders replaced
	 */
	public static Component parse(String text, String... placeholders) {
		...
		if (placeholders.length % 2 != 0) {
			throw new IllegalArgumentException(
					"Placeholders must be in key-value pairs");
		}

		List<TagResolver> resolvers = new ArrayList<>();
		for (int i = 0; i < placeholders.length; i += 2) {
			resolvers.add(
					Placeholder.parsed(placeholders[i], placeholders[i + 1]));
		}
	 ...
	 * @param includeWorld
	 *            Whether to include the world name
	...
	public static Component formatLocation(Location location,
		...
		if (includeWorld && location.getWorld() != null) {
			displayText = "<grey>World: " + location.getWorld().getName()
					+ " | " + displayText;

			hoverBuilder.append(Component.newline());
			hoverBuilder.append(parse("<yellow>World:</yellow> "
					+ location.getWorld().getName()));
		}

		hoverBuilder.append(Component.newline());
		hoverBuilder.append(parse("<gray>Yaw:</gray> "
				+ String.format("%.2f", location.getYaw())));
		hoverBuilder.append(Component.newline());
		hoverBuilder.append(parse("<gray>Pitch:</gray> "
	...
	public static Component formatItem(ItemStack item) {
		...
		if (item.hasItemMeta() && item.getItemMeta().hasDisplayName()) {
			hoverBuilder.append(item.getItemMeta().displayName()
		...
		String displayText = "<aqua>" + itemName
